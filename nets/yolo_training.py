...
def cross_IOU(_box_a, _box_b):
    ...
    min_xy = torch.max(box_a[:, :2].unsqueeze(1).expand(A, B, 2),
                       box_b[:, :2].unsqueeze(0).expand(A, B, 2))
    # 当inter为负时表明_box_a, _box_b不相交，将负值设为0
...
def box_ciou(b1, b2):
    ...
    # v = pi^2/4 * [arctan(wg / hg) - arctan(w / h)]^2
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets = None):
        ...
        # targets 为训练集的targets, x,y,w,h,class
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        # 计算一共有多少张图片
        bs = len(target)
        # 匹配先验框，大中小三类框匹配13,26,52三种尺寸的特征图
        anchor_index = [[0, 1, 2], [3, 4, 5], [6, 7, 8]][self.feature_length.index(in_w)]
        subtract_index = [0, 3, 6][self.feature_length.index(in_w)]
        anchor_quantity = int(self.num_anchors / 3)
        # 创建全是0或者全是1的阵列
        '''
        bs = 第几张图片
        int(num_anchors / 3) = 几种anchor（可以直接设为3，因为9种尺寸的anchor，3种大小的特征图，每类特征图有3个对应尺寸的anchor）
        in_h, in_w = 特征图尺寸（其实in_w = in_h）
        num_classes = 种类总数
        '''

        # 有obj为1，无obj为0
        mask = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        # 无obj为1，有obj为0
        noobj_mask = torch.ones(bs, anchor_quantity, in_h, in_w, requires_grad = False)

        # GT在特征层上的x,y,w,h
        tx = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        ty = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        tw = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        th = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        
        # GT在特征层上的x,y,w,h合集
        t_box = torch.zeros(bs, anchor_quantity, in_h, in_w, 4, requires_grad = False)
        
        # 存在obj的置信度
        tconf = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        
        # 存在obj时，该obj所属种类
        tcls = torch.zeros(bs, anchor_quantity, in_h, in_w, self.num_classes, requires_grad = False)

        # GT本身的w,h（不乘in_w, in_h）
        box_loss_scale_x = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        box_loss_scale_y = torch.zeros(bs, anchor_quantity, in_h, in_w, requires_grad = False)
        ...
        for b in range(bs):
            ...
            # 拼接0,0,gws,ghs得到GT的尺寸
            # 0,0是为了匹配anchor只有宽高尺寸，没有中心坐标
            # 用0,0相当于把GT和anchor全部放到原点，方便后续计算
            gt_box = torch.FloatTensor(torch.cat([torch.zeros_like(gws), torch.zeros_like(ghs), gws, ghs], 1))
            
            # 拼接所有anchor的尺寸0,0,w,h
            anchor_shapes = torch.FloatTensor(torch.cat((torch.zeros((self.num_anchors, 2)), torch.FloatTensor(anchors)), 1))
            
            # 计算每个GT和每个anchor重合程度IOU (n * 9, n为该图片上GT的数量)
            anch_ious = cross_IOU(gt_box, anchor_shapes)

            # GT和哪个anchor的IOU最大，这个anchor的索引
            best_ns = torch.argmax(anch_ious, dim = -1)
            for i, best_n in enumerate(best_ns):
                # 如果anchor的索引不在这个特征图对应的anchor的范围内，跳过
                if best_n not in anchor_index:
                    continue

                # 第i个GT位于哪个grid cell (gj, gi)
                # gi,gj反过来是因为坐标和数组的索引是相反的
                gi = gis[i].long()
                gj = gjs[i].long()
                ...
                # 判断(gj, gi)在(in_h, in_w)内
                if (gj < in_h) and (gi < in_w):
                    # [0, 1, 2] - 0    [3, 4, 5] - 3    [6, 7, 8] - 6
                    best_n = best_n - subtract_index

                    # 判定哪个网格内部存在物体
                    # 第b张图片，匹配第best_n个anchor的GT，(gj, gi)个网格内有obj，值改为0
    ...
    def get_ignore(self, prediction, target, scaled_anchors, in_w, in_h, noobj_mask):
        ...
        for i in range(bs):
            ...
            if len(target[i]) > 0:
                ...
                anch_ious = cross_IOU(gt_box, pred_boxes_for_ignore)
